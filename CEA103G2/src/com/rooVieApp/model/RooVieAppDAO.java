...
//	COUNTQUESTION用在insert需要幾個?(不含PK)
	...
	//針對個別物件SHOW出全部
	private static final String LISTALLPICKTIME = "SELECT " + TOTAL_COL + " FROM " + TABLE + " WHERE HOS_NO=? and (RVA_STATUS=0 or RVA_STATUS=1) and DATEDIFF(RVA_ORDER_TIME,NOW()) > 0  order by RVA_ORDER_TIME";
	//針對個別物件SHOW出全部(未指派)
	private static final String LISTALLPICKTIME_NOASSIGN = "SELECT " + TOTAL_COL + " FROM " + TABLE + " WHERE HOS_NO=? and EMP_NO is NULL and RVA_STATUS=0 and DATEDIFF(RVA_ORDER_TIME,NOW()) > 0  order by RVA_ORDER_TIME";
	...
	private static final String GETEMPAPP = "SELECT * FROM room_viewing_application where emp_no=? and RVA_STATUS=1";
...
//			否則這裡顯示錯誤就處理掉了，但前台都沒發生報錯
	...
	public List<RooVieAppVO> listOrderTime(RooVieAppVO rooVieAppVO) {
		...
		try {
			...
			con = ds.getConnection();
			pstmt = con.prepareStatement(LISTORDERTIME);
			pstmt.setByte(1, rooVieAppVO.getRva_status());
			pstmt.setInt(2, rooVieAppVO.getEmp_no());
			pstmt.setInt(3, rooVieAppVO.getHos_no());
			rs = pstmt.executeQuery();
...
//			否則這裡顯示錯誤就處理掉了，但前台都沒發生報錯
	...
	public List<RooVieAppVO> showTheEmpApp(Integer emp_no , Byte status) {
		...
		try {
			...
			con = ds.getConnection();
			pstmt = con.prepareStatement(SHOWTHEEMPAPP);
			pstmt.setInt(1, emp_no);
...
//			否則這裡顯示錯誤就處理掉了，但前台都沒發生報錯
	...
	public List<RooVieAppVO> showTheMemApp(Integer mem_no , Byte status) {
		...
		try {
			...
			con = ds.getConnection();
			pstmt = con.prepareStatement(SHOWTHEMEMAPP);
