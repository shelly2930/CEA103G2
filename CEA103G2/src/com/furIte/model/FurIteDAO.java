...
public class FurIteDAO implements FurIteDAO_interface {
	...
	private static final String INSERT_STMT = 
	    ...
	    "fnt_length,fnt_width,fnt_height,fnt_weight,fnt_standard,fnt_info,fnt_views)" + 
		" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//		"DELETE FROM FURNITURE_ITEM where fnt_it_no = ?";
	...
	private static final String UPDATE = 
		...
		"fnt_length= ?,fnt_width= ?,fnt_height= ?,fnt_weight= ?,fnt_standard= ?,fnt_info= ?,fnt_views= ? where fnt_it_no = ?";
	//取最後一筆品項編號作法
	...
	public void update(FurIteVO furIteVO) {
		...
		try {
			...
			pstmt.setString(12, furIteVO.getFnt_info());
			pstmt.setInt(13, furIteVO.getFnt_views());
			pstmt.setInt(14, furIteVO.getFnt_it_no());
